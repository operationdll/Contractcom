...
<html lang="{{ config('app.locale') }}">
                    ...
                    $.getJSON("{{url('person/list')}}",{'name':$("#searchValue").val()},function(data){
                        ...
                        $.each(data,function(i,item){
                            $html = $html+'<tr><td>'+item.F_name+' '+item.L_name+'</td>'
                                    ...
                                    +'<td>'+item.city+'</td>'
                                    +'<td>'+item.address+'</td>'
                                    +'<td><a href="javascript:showImg(\''+item.photo+'\')">'+item.photo+'</a></td>'
    ...
    <div class="container">
        ...
        <div class="container">
            <h2>Person information<button class="btn btn-primary my-2 my-sm-0" type="button" id="addBnt">Add</button></h2>
            <table class="table">
                <thead>
                <tr>
                    <th>Name</th>
                ...
                @foreach($persons as $p)
                    <tr>
                        ...
                        <td>
                            <a href="javascript:showImg('{{$p->photo}}')">{{$p->photo}}</a>
                        </td>
                        <td>
                            {{$p->short_desc}}
                        </td>
